document.addEventListener('DOMContentLoaded', () => {
  ...
  const replay = document.querySelector('.restart-btn')
  ...
  function clearBoard() {
    // Remove all blocks (both active and frozen)
    squares.forEach(square => {
      square.classList.remove('block', 'block2')
      square.style.backgroundImage = 'none'
    })

    // Make sure bottom row remains as block3
    for (let i = 0; i < GRID_WIDTH; i++) {
      squares[GRID_SIZE + i].classList.add('block3')
    }
  }
